...
# 修改: SimpleImageManager （并发、去重、哈希、重试）
import aiohttp, aiofiles, asyncio, hashlib, mimetypes
from storage import get_image_by_url, get_image_by_hash, upsert_image

class SimpleImageManager:
    def __init__(self, storage_path: str = "./images", base_url: str = "http://localhost:8000/images"):
        self.storage_path = Path(storage_path); self.storage_path.mkdir(exist_ok=True)
    ...
    async def download_images(self, urls: List[str], business_id: str) -> Dict[str, str]:
        ...
        sem = asyncio.Semaphore(8)  # 并发控制
        tasks = [self._ensure_one(url, business_id, business_dir, sem) for url in dict.fromkeys(urls)]
        results = await asyncio.gather(*tasks)
        return {r["original_url"]: r["public_url"] for r in results if r}

    async def _ensure_one(self, url: str, biz: str, biz_dir: Path, sem: asyncio.Semaphore):
        # 命中缓存（按 original_url）
        cached = await get_image_by_url(url)
        if cached and cached.get("public_url"):
            print(f"缓存命中: {url}")
            return cached

        async with sem:
            # 下载 + 重试
            content, ctype = await self._fetch_with_retry(url, tries=2, timeout=12)
            if not content:
                rec = {"business_id": biz, "original_url": url, "status": "fail"}
                await upsert_image(rec); return rec

            sha = hashlib.sha256(content).hexdigest()
            # 命中同内容文件（按 sha256）
            same = await get_image_by_hash(sha)
            if same and same.get("local_path"):
                local_path = Path(same["local_path"])
                # 直接复用
                public_url = f"{self.base_url}/{biz}/{local_path.name}"
                rec = {"business_id": biz, "original_url": url, "local_path": str(local_path),
                       "public_url": public_url, "sha256": sha, "bytes": len(content), "status": "ok"}
                await upsert_image(rec); return rec

            # 新文件落盘
            ext = self._ext_from_content_type(ctype) or self._ext_from_url(url) or ".jpg"
            fname = hashlib.md5(url.encode()).hexdigest()[:12] + ext
            fpath = biz_dir / fname
            async with aiofiles.open(fpath, "wb") as f:
                await f.write(content)

            public_url = f"{self.base_url}/{biz}/{fname}"
            rec = {"business_id": biz, "original_url": url, "local_path": str(fpath),
                   "public_url": public_url, "sha256": sha, "bytes": len(content), "status": "ok"}
            await upsert_image(rec); return rec

    async def _fetch_with_retry(self, url: str, tries: int, timeout: int):
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as s:
            for attempt in range(1, tries+1):
                try:
                    ...
                    print(f"下载失败({attempt}/{tries}): {url} -> {e}")
                await asyncio.sleep(0.6 * attempt)
        return None, None

    def _ext_from_content_type(self, ctype: str) -> Optional[str]:
        if not ctype: return None
        ext = mimetypes.guess_extension(ctype.split(";")[0].strip())
        # 某些服务器返回 image/jpg，需要规范化
        return ".jpg" if ext in (".jpe",".jpeg") else ext

    def _ext_from_url(self, url: str) -> Optional[str]:
        return Path(url.split("?")[0]).suffix or None
